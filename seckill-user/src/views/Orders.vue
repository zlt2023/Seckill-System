<template>
  <div class="orders-page">
    ...
    <div class="filter-tabs animate-fadeInUp" style="animation-delay: 0.1s">
      ...
      >
        待支付
        <span class="tab-badge" v-if="stats?.unpaid">{{ stats.unpaid }}</span>
      </button>
      <button
        class="filter-tab"
    ...
    <div
      v-else
      class="order-list animate-fadeInUp"
        ...
        <div class="order-body">
          ...
          <div class="order-meta">
            <span class="price-tag"
              ><span class="symbol">¥</span>{{ order.goodsPrice }}</span
            >
            <span class="order-time">{{ formatTime(order.createTime) }}</span>
          </div>
        </div>
        <div class="order-footer" v-if="order.status === 0">
          <span class="pay-countdown" v-if="order._countdown"
            >⏰ {{ order._countdown }} 后关闭</span
          >
...
async function loadStats() {
  try {
    const res: any = await orderApi.stats();
