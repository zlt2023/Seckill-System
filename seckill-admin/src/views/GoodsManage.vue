      ...
      <div class="goods-grid animate-fadeInUp" style="animation-delay: 0.1s">
        ...
        >
          ...
          <div class="goods-price">
            <span class="price"
              ><span class="symbol">¥</span>{{ item.seckillPrice }}</span
            >
          </div>
          ...
          <div class="goods-stock-row">
            ...
            <div class="stock-item">
              ...
              <span
                class="stock-number"
                :class="{
                  warn: item.redisStock <= 10,
                  mismatch: item.dbStock !== item.redisStock,
                }"
              >
                {{ item.redisStock }}
              </span>
      ...
      >
        ...
        <el-form-item label="商品标题" prop="goodsTitle">
          <el-input
            ...
            placeholder="例如: 全新未拆封沙丘金"
          />
        </el-form-item>
        <el-form-item label="商品图片" prop="goodsImg">
          <el-input
            v-model="formData.goodsImg"
            placeholder="图片URL，如果为空可留白"
          />
        </el-form-item>
...
async function openEditDialog(item: any) {
  ...
  // 商品本尊的状态 (对应 dbStatus 或 goodsStatus)
  formData.status = item.goodsStatus === 0 ? 0 : 1;
...
async function deleteGoods(item: any) {
  try {
    await ElMessageBox.confirm(
      `确定要删除秒杀商品「${item.goodsName}」吗？操作不可逆。`,
      "删除确认",
      {
        type: "warning",
